...
def main(argv):
	with CytomineJob.from_cli(argv) as cj:
		...
		tr_imgs = []
    	for image in images:
    		if image.numberOfAnnotations == len(terms_collection):
        		tr_imgs.append(image)
		...
		for image in tr_images:
			...
			for ann in annotations:
				...
				terms[ann.term[0]] = 1
			#Download landmarks:
				F = open(landmarks_path + '%d.txt' % image.id, 'w')
				for t in terms.keys():
					if (t, image.id) in xpos:
						F.write('%d %d\n' % (
						xpos[(t, image.id)], ypos[(t, image.id)]))
				F.close()
		...
		org_images = glob.glob(images_path+'*.png')
		...
		train_idxs, test_idxs = train_test_split(idxs, test_size=0.1, random_state=seed)

		train_idxs, val_idxs = train_test_split(train_idxs, test_size=0.1, random_state=seed)
