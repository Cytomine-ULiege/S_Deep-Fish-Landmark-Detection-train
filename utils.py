...
def rescale_pad_img(image, landmarks, desired_size):
    ...
    if aspect > 1 : #horizontal image
        ...
        img = cv.copyMakeBorder(re_img, top, bottom, 0,0, cv.BORDER_REPLICATE)
        x = landmarks[:,0]
        y = landmarks[:,1]
        new_x = x * new_w / w
        ...
        dim = (new_w, new_h)
        re_img = cv.resize(image, dim, interpolation = cv.INTER_AREA)
        img = cv.copyMakeBorder(re_img, top, bottom, 0,0, cv.BORDER_CONSTANT, value=0)
    ...
    return img, np.vstack((new_x, new_y)).T
